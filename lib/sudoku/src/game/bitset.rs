use crate::prelude::{Value, ValueOption};

/// A simple bitset for storing regular Sudoku-sized (i.e., up to 81) index values.
///
/// ## Technical Notes
/// Practically this implementation allows for storing up to 127 different indexes.
#[derive(Debug, Default, Copy, Clone, Eq, PartialEq, Ord, PartialOrd, Hash)]
pub struct IndexBitSet {
    /// We anticipate at most 81 fields on a standard Sudoku game.
    /// We use a 128-bit type here to directly encode the field values,
    /// even though this wastes 47 bits.
    state: u128,
}

impl IndexBitSet {
    #[inline]
    pub const fn with_value(mut self, value: Value) -> Self {
        let value = value.get() as u128;
        // Since the value is a non-zero u8 we subtract one for the first bit.
        self.state |= 1 << (value - 1);
        self
    }

    #[inline]
    pub fn insert(&mut self, value: Value) -> &mut Self {
        let value = value.get() as u128;
        // Since the value is a non-zero u8 we subtract one for the first bit.
        self.state |= 1 << (value - 1);
        self
    }

    #[inline]
    pub const fn without_value(mut self, value: Value) -> Self {
        let value = value.get() as u128;
        // Since the value is a non-zero u8 we subtract one for the first bit.
        self.state &= !(1 << (value - 1));
        self
    }

    #[inline]
    pub fn remove(&mut self, value: Value) -> &mut Self {
        let value = value.get() as u128;
        // Since the value is a non-zero u8 we subtract one for the first bit.
        self.state &= !(1 << (value - 1));
        self
    }

    #[inline]
    pub const fn with_union(mut self, other: &IndexBitSet) -> Self {
        self.state |= other.state;
        self
    }

    #[inline]
    pub fn union(&mut self, other: &IndexBitSet) -> &mut Self {
        self.state |= other.state;
        self
    }

    #[inline]
    pub const fn contains(&self, value: &Value) -> bool {
        let value = value.get() as u128;
        // Since the value is a non-zero u8 we subtract one for the first bit.
        let flag = self.state & (1 << (value - 1));
        flag != 0
    }

    pub fn len(&self) -> usize {
        self.state.count_ones() as _
    }

    pub fn is_empty(&self) -> bool {
        self.state == 0
    }
}

impl From<&[u8]> for IndexBitSet {
    #[inline]
    fn from(values: &[u8]) -> Self {
        let mut state = 0u128;
        for value in values {
            debug_assert_ne!(*value, 0);
            // Since the value is a non-zero u8 we subtract one for the first bit.
            state |= 1 << (value - 1);
        }
        Self { state }
    }
}

impl From<&[Value]> for IndexBitSet {
    #[inline]
    fn from(values: &[Value]) -> Self {
        let mut state = 0u128;
        for value in values {
            // Since the value is a non-zero u8 we subtract one for the first bit.
            state |= 1 << (value.get() - 1);
        }
        Self { state }
    }
}

impl From<&[ValueOption]> for IndexBitSet {
    #[inline]
    fn from(values: &[ValueOption]) -> Self {
        let mut state = 0u128;
        for value in values {
            if let Some(value) = value {
                // Since the value is a non-zero u8 we subtract one for the first bit.
                state |= 1 << (value.get() - 1);
            }
        }
        Self { state }
    }
}

#[cfg(test)]
mod tests {
    use crate::game::bitset::IndexBitSet;
    use crate::prelude::Value;

    #[test]
    fn with_value() {
        let a = Value::try_from(81).unwrap();
        let b = Value::try_from(17).unwrap();
        let c = Value::try_from(2).unwrap();

        let bitset = IndexBitSet::default().with_value(a).with_value(b);

        assert!(bitset.contains(&a));
        assert!(bitset.contains(&b));
        assert!(!bitset.contains(&c));

        assert_eq!(bitset.len(), 2);
        assert!(!bitset.is_empty());
    }

    #[test]
    fn union() {
        let a = Value::try_from(81).unwrap();
        let b = Value::try_from(17).unwrap();
        let c = Value::try_from(2).unwrap();

        let bitset_a = IndexBitSet::default().with_value(a);
        let bitset_b = IndexBitSet::default().with_value(b);
        let bitset = bitset_a.with_union(&bitset_b);

        assert!(bitset.contains(&a));
        assert!(bitset.contains(&b));
        assert!(!bitset.contains(&c));
    }

    #[test]
    fn without_value() {
        let a = Value::try_from(81).unwrap();
        let b = Value::try_from(17).unwrap();
        let c = Value::try_from(2).unwrap();

        let bitset = IndexBitSet::default()
            .with_value(a)
            .with_value(b)
            .with_value(c);
        let bitset = bitset.without_value(a).without_value(b);

        assert!(!bitset.contains(&a));
        assert!(!bitset.contains(&b));
        assert!(bitset.contains(&c));
    }

    #[test]
    fn from_value_slice() {
        let a = Value::try_from(81).unwrap();
        let b = Value::try_from(17).unwrap();
        let c = Value::try_from(2).unwrap();

        let bitset = IndexBitSet::from([a, b].as_slice());

        assert!(bitset.contains(&a));
        assert!(bitset.contains(&b));
        assert!(!bitset.contains(&c));
    }

    #[test]
    fn from_u8_slice() {
        let a = Value::try_from(81).unwrap();
        let b = Value::try_from(17).unwrap();
        let c = Value::try_from(2).unwrap();

        let bitset = IndexBitSet::from([a.get(), b.get()].as_slice());

        assert!(bitset.contains(&a));
        assert!(bitset.contains(&b));
        assert!(!bitset.contains(&c));
    }
}
